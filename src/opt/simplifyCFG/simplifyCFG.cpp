...
void SimplifyCFG::simplifyCFG(ir::Function *func) {
...
void SimplifyCFG::mergeBlocks(ir::Function *func,
...
bool SimplifyCFG::foldTrivialPhisInBlock(ir::BasicBlock &bb) {
  bool changed = false;
  auto &insts = bb.instructions();
  for (std::size_t i = 0; i < insts.size();) {
    auto phi = std::dynamic_pointer_cast<ir::PhiInst>(insts[i]);
...
void SimplifyCFG::rebuildPredecessors(ir::Function &function) {
