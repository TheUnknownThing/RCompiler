...
std::string cfg_to_string(const ir::Function &fn) {
  ...
  const auto index = detail::index_blocks(fn);
  oss << " (" << blocks.size() << " blocks)\n";

  for (std::size_t i = 0; i < blocks.size(); ++i) {
    const auto &bb = *blocks[i];
    oss << "- " << detail::node_label(bb, i) << "\n";
    ...
    // Predecessors
    {
      ...
      preds = detail::dedup_and_sort_by_index(std::move(preds), index);
    ...
    // Successors (computed from terminator)
    {
      ...
      succs = detail::dedup_and_sort_by_index(std::move(succs), index);
...
std::string cfg_to_dot(const ir::Function &fn) {
  ...
  oss << "digraph CFG {\n";
  oss << "  labelloc=\"t\";\n";
  oss << "  label=\"CFG: " << detail::escape_dot_label(fn.name()) << "\";\n";
  oss << "  node [shape=box, fontname=\"monospace\"];\n";

  for (std::size_t i = 0; i < blocks.size(); ++i) {
    const auto &bb = *blocks[i];
    oss << "  " << detail::node_id(i) << " [label=\""
        << detail::escape_dot_label(detail::node_label(bb, i)) << "\"];\n";
  }
  ...
  for (std::size_t i = 0; i < blocks.size(); ++i) {
    ...
    for (const auto *s : succs) {
      ...
      oss << "  " << detail::node_id(i) << " -> " << detail::node_id(it->second)
          << ";\n";
    }
  }
