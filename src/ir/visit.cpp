...
void IREmitter::pushOperand(ValuePtr v) {
  if (!v) {
...
ValuePtr IREmitter::createAlloca(const TypePtr &ty,
...
ValuePtr IREmitter::loadPtrValue(ValuePtr v, const SemType &semTy) {
  if (!v) {
...
bool IREmitter::typeEquals(const TypePtr &a, const TypePtr &b) const {
  if (a->kind() != b->kind())
    return false;
  switch (a->kind()) {
  case TypeKind::Void:
...
ValuePtr IREmitter::lookupLocal(const std::string &name) const {
...
void IREmitter::bindLocal(const std::string &name, ValuePtr v) {
  if (locals_.empty()) {
    locals_.emplace_back();
  }
...
bool IREmitter::isInteger(SemPrimitiveKind k) const {
...
std::optional<BinaryOpKind> IREmitter::tokenToOP(TokenType tt) const {
  switch (tt) {
  case TokenType::PLUS:
  case TokenType::PLUS_EQ:
...
std::string IREmitter::getScopeName(const ScopeNode *scope) const {
...
ValuePtr IREmitter::findAggregateInitTarget(const TypePtr &ty) const {
...
void IREmitter::pushAggregateInitTarget(const TypePtr &ty,
                                               ValuePtr ptr) {
  aggregate_init_targets_.push_back({ty, ptr});
}
void IREmitter::popAggregateInitTarget() {
...
TypePtr IREmitter::getParamType(const SemType &paramSemTy) const {
...
char IREmitter::decodeCharLiteral(const std::string &literal) {
...
std::string IREmitter::decodeStringLiteral(const std::string &literal) {
  if (literal.empty()) {
    return {};
  }
...
FuncPtr IREmitter::createMemsetFn() {
...
FuncPtr IREmitter::createMemcpyFn() {
...
std::size_t IREmitter::computeTypeByteSize(const TypePtr &ty) const {
  return computeTypeLayout(ty).size;
}
bool IREmitter::isZeroLiteral(const Expression *expr) const {
...
bool IREmitter::isAggregateType(const TypePtr &ty) const {
  return ty->kind() == TypeKind::Array || ty->kind() == TypeKind::Struct;
}
void IREmitter::emitMemcpy(ValuePtr dst, ValuePtr src,
                                  std::size_t byteSize) {
  if (byteSize == 0) {
    return;
  }
