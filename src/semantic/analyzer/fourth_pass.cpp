...
void FourthPass::run(const std::shared_ptr<RootNode> &root,
...
void FourthPass::visit(LetStatement &node) {
...
void FourthPass::visit(CallExpression &node) {
  if (auto *name_expr =
    ...
    for (auto *s = current_scope_node; s; s = s->parent) {
      if (auto *it = s->find_value_item(name_expr->name)) {
    ...
    if (!fn) {
      throw SemanticException("unknown function '" + name_expr->name + "'");
    }
    ...
    resolve_path_function_call(*path_expr, node);
    return;
  }
...
SemType FourthPass::eval_binary(BinaryExpression &bin) {
  ...
  throw TypeError("operator '" + std::string(to_string(op)) +
...
void FourthPass::handle_assignment(BinaryExpression &node) {
  ...
  if (!ok) {
    throw TypeError("invalid types for compound assignment: '" +
                    to_string(lhs_t) + "' " + std::string(to_string(base_op)) +
                    "= '" + to_string(rhs_t) + "'");
  }
