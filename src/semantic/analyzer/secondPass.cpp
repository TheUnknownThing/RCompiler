...
SecondPassResolver::SecondPassResolver()
    : root_scope(nullptr), evaluator() {}
void SecondPassResolver::run(const std::shared_ptr<RootNode> &root,
...
void SecondPassResolver::enterScope(ScopeNode *s) {
  if (s) {
    scope_stack.push_back(s);
  }
}
void SecondPassResolver::exitScope() {
