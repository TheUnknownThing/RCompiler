#include "backend/passes/instSelect.hpp"
...
void InstructionSelection::visit(const ir::BasicBlock &basicBlock) {
...
void InstructionSelection::visit(const ir::BinaryOpInst &binOp) {
...
void InstructionSelection::visit(const ir::ZExtInst &zextInst) {
...
void InstructionSelection::visit(const ir::SExtInst &sextInst) {
...
void InstructionSelection::visit(const ir::TruncInst &truncInst) {
...
void InstructionSelection::visit(const ir::MoveInst &moveInst) {
  // addi rd, rs, 0
...
std::shared_ptr<Register> InstructionSelection::createVirtualRegister() {
...
bool InstructionSelection::isAggregateType(const ir::TypePtr &ty) const {
  return ty->kind() == ir::TypeKind::Array ||
         ty->kind() == ir::TypeKind::Struct;
}
size_t InstructionSelection::alignTo(size_t offset, size_t align) const {
  if (align <= 1) {
    return offset;
  }
...
std::shared_ptr<AsmOperand> InstructionSelection::prepareCallArgument(
