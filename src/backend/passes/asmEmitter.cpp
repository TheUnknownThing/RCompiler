#include "backend/passes/asmEmitter.hpp"
...
std::string AsmEmitter::opcodeName(InstOpcode opcode) const {
  switch (opcode) {
  case InstOpcode::ADD:
    return "add";
  case InstOpcode::SUB:
...
bool AsmEmitter::fitsSigned12(int64_t value) const {
  return value >= -2048 && value <= 2047;
}
void AsmEmitter::emitLoad(
...
void AsmEmitter::emitInst(const AsmFunction &function,
                                 const AsmInst &inst,
                                 std::ostream &os) const {
