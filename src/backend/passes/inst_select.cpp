...
void InstructionSelection::visit(const ir::BasicBlock &basic_block) {
  ...
  if (&basic_block == entry_ir_block_ && current_function_) {
    ...
    const size_t register_arg_count = std::min<size_t>(8, current_function_->args().size());
    ...
    for (size_t i = 0; i < current_function_->args().size(); ++i) {
      ...
      if (i < 8) {
        current_block_->create_inst(InstOpcode::LW, dst, {register_arg_slots[i]});
        continue;
      }

      auto slot = create_incoming_arg_slot(i);
      size_t arg_size = compute_type_byte_size(current_function_->args()[i]->type());
      if (arg_size == 1) {
...
void InstructionSelection::visit(const ir::BinaryOpInst &bin_op) {
  ...
  case rc::ir::BinaryOpKind::ADD: {
    ...
    if (lhs_imm && rhs_imm) {
      current_block_->create_inst(
          InstOpcode::LI, dst,
          {create_immediate(static_cast<int32_t>(lhs_imm->value +
                                                rhs_imm->value))});
      break;
    }
    current_block_->create_inst(InstOpcode::ADD, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  case rc::ir::BinaryOpKind::SUB: {
    if (auto *imm = as_immediate(rhs); imm) {
      ...
      if (neg64 >= -2048 && neg64 <= 2047) {
        ...
        current_block_->create_inst(InstOpcode::ADDI, dst, {get_reg(lhs), neg_imm});
        break;
      }
    }
    current_block_->create_inst(InstOpcode::SUB, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  case rc::ir::BinaryOpKind::MUL: {
    current_block_->create_inst(InstOpcode::MUL, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  case rc::ir::BinaryOpKind::SDIV: {
    current_block_->create_inst(InstOpcode::DIV, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  ...
  case rc::ir::BinaryOpKind::SREM: {
    current_block_->create_inst(InstOpcode::REM, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  ...
  case rc::ir::BinaryOpKind::SHL: {
    ...
    current_block_->create_inst(InstOpcode::SLL, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  case rc::ir::BinaryOpKind::ASHR: {
    ...
    current_block_->create_inst(InstOpcode::SRA, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  case rc::ir::BinaryOpKind::LSHR: {
    ...
    current_block_->create_inst(InstOpcode::SRL, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  case rc::ir::BinaryOpKind::AND: {
    ...
    current_block_->create_inst(InstOpcode::AND, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  case rc::ir::BinaryOpKind::OR: {
    ...
    current_block_->create_inst(InstOpcode::OR, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  case rc::ir::BinaryOpKind::XOR: {
    ...
    current_block_->create_inst(InstOpcode::XOR, dst, {get_reg(lhs), get_reg(rhs)});
    break;
  }
  default:
...
void InstructionSelection::visit(const ir::LoadInst &load) {
  ...
  auto ptr = resolve_operand_or_immediate(load.pointer(),
...
void InstructionSelection::visit(const ir::StoreInst &store) {
  ...
  if (slot) {
    if (store_size == 1) {
      current_block_->create_inst(InstOpcode::SB, nullptr, {get_reg(src), slot});
    } else if (store_size == 2) {
      current_block_->create_inst(InstOpcode::SH, nullptr, {get_reg(src), slot});
    } else if (store_size == 4) {
      current_block_->create_inst(InstOpcode::SW, nullptr, {get_reg(src), slot});
    } else {
...
void InstructionSelection::visit(const ir::GetElementPtrInst &gep) {
  ...
  for (const auto &index : gep.indices()) {
    ...
    if (current_type->kind() == ir::TypeKind::Array) {
      auto arr_ty = std::dynamic_pointer_cast<const ir::ArrayType>(current_type);
      if (!arr_ty) {
    ...
    if (current_type->kind() == ir::TypeKind::Struct) {
      ...
      for (size_t field_index = 0; field_index < static_cast<size_t>(idx_value);
           ++field_index) {
        auto field_layout = compute_type_layout(struct_ty->fields()[field_index]);
  ...
  if (slot && !dynamic_offset && static_offset >= 0) {
    ...
    if (existing_it != value_operand_map_.end() && existing_it->second) {
      auto existing_reg = std::dynamic_pointer_cast<Register>(existing_it->second);
      if (!existing_reg) {
...
InstructionSelection::materialize_address(const std::shared_ptr<AsmOperand> &ptr,
...
void
InstructionSelection::emit_add_immediate(const std::shared_ptr<Register> &dst,
  ...
  current_block_->create_inst(InstOpcode::ADD, dst, {get_reg(lhs), get_reg(imm)});
}
...
void InstructionSelection::visit(const ir::BranchInst &branch) {
  if (branch.is_conditional()) {
    ...
    current_block_->create_inst(InstOpcode::BNEZ, nullptr,
                              {get_reg(cond), create_label_operand(branch.dest()),
...
void InstructionSelection::visit(const ir::ICmpInst &icmp) {
  ...
  auto lhs =
      resolve_operand_or_immediate(icmp.lhs(), "LHS operand for ICmpInst", &icmp);
  auto rhs =
      resolve_operand_or_immediate(icmp.rhs(), "RHS operand for ICmpInst", &icmp);
  switch (icmp.pred()) {
  case ir::ICmpPred::EQ:
    // rd = (rs1 ^ rs2) == 0
    current_block_->create_inst(InstOpcode::XOR, dst, {get_reg(lhs), get_reg(rhs)});
  ...
  case ir::ICmpPred::SGT:
    current_block_->create_inst(InstOpcode::SLT, dst, {get_reg(rhs), get_reg(lhs)});
    break;
  ...
  case ir::ICmpPred::SLT:
    current_block_->create_inst(InstOpcode::SLT, dst, {get_reg(lhs), get_reg(rhs)});
    break;
...
void InstructionSelection::visit(const ir::CallInst &call) {
  ...
  current_block_->create_inst(InstOpcode::CALL, nullptr,
  ...
  if (!call.type()->is_void()) {
    current_block_->create_inst(InstOpcode::MV, get_or_create_result_register(&call),
...
void InstructionSelection::visit(const ir::ZExtInst &zext_inst) {
  ...
  auto src_int =
      std::dynamic_pointer_cast<const ir::IntegerType>(zext_inst.source()->type());
...
std::shared_ptr<AsmOperand>
InstructionSelection::resolve_operand_or_immediate(
...
std::shared_ptr<AsmOperand> InstructionSelection::prepare_call_argument(
  ...
  auto operand =
      resolve_operand_or_immediate(arg, "call argument not materialized", &call);
