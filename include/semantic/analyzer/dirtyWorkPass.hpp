...
public:
  ...
  void visit(CallExpression &node) override {
    ...
    if (nameExpr && nameExpr->name == "exit") {
      if (!current_function_name.has_value() || current_function_name.value() != "main") {
        throw SemanticException("exit() function can only be called from main function");
      }
    ...
    if (current_function_name.has_value() && current_function_name.value() == "main") {
      ...
      if (found_exit_call) {
        ...
        if (node.final_expr.has_value()) {
          auto *final_call = dynamic_cast<CallExpression *>(node.final_expr.value().get());
          if (final_call) {
            auto *final_name = dynamic_cast<NameExpression *>(final_call->function_name.get());
            if (final_name && final_name->name == "exit") {
              is_exit_final = true;
            }
          }
        } else if (!node.statements.empty()) {
          auto *last_stmt = dynamic_cast<ExpressionStatement *>(node.statements.back().get());
          if (last_stmt && last_stmt->expression) {
            auto *last_call = dynamic_cast<CallExpression *>(last_stmt->expression.get());
            if (last_call) {
              auto *last_name = dynamic_cast<NameExpression *>(last_call->function_name.get());
        ...
        if (!is_exit_final) {
          throw SemanticException("exit() must be the final statement in main function");
        }
      }
    }
